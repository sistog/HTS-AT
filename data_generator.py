...
class DeepShip_Dataset(Dataset):
    def __init__(self, config, eval_mode = False):
        self.config = config
        self.eval_mode = eval_mode
        if self.eval_mode:
            file_root = "/data/zcx/wav_prj/Qiandao/src/datafiles/deepship_eval_data.json"
        else:
            file_root = "/data/zcx/wav_prj/Qiandao/src/datafiles/deepship_train_data.json"
    ...
    def __getitem__(self, index):
        ...
        """
        p = self.queue[index]
        file_path = self.data[p]['wav']
        name = os.path.basename(file_path)
        waveform, _ = librosa.load(file_path, sr=None)
        label = self.data[p]['labels']
        CLASS_DICT = {'Cargo':0, 'Passengership':1, 'Tanker':2, "Tug":3}
        target = CLASS_DICT[label]
        data_dict = {
            "audio_name": name,
            "waveform": waveform,
